...
= Brief roadmap and TODO  
  ...
  1) Recognizing filters in dm model 
    ...
    a block that gets read as maybe
     ...
     To be precise, a way to build an object specific to access by apply_filters

   2) proper datamapper syntax for self.all methods
   3) passing url params to model
   4) recognizing and returning results

  
= has_filter

has_filter provides a controller based interface for building and applying multiple filters to a DataMapper model.
...
use:
   ...
   See results from your application in a browser:
      ...
      #=> all tickets within the speficied period     
